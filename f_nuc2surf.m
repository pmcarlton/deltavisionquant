#!/usr/bin/octave -q
function [surfs,nuccenters,maxdapi,mpts]=f_nuc2surf(DAPI);
#f_nuc2surf.m -- function for finding nuclei centers, 2013-06-10pmc

pixborder=5; % remove pixels this close to the edge first
W=3;HW=floor(W/2);
pixsize=[.064 .064 .2];

DAPI(1:pixborder,:,:)=0;
DAPI(end-(pixborder-1):end,:,:)=0;
DAPI(:,1:pixborder,:)=0;
DAPI(:,end-(pixborder-1):end,:)=0;
