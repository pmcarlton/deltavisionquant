function q=mrcLoad(fn)

%returns everything in the struct `q` instead of umpteen return values, 
%20190401pmc
...
dtypes={'uint8','int16','single','int16','single','int16','uint16','int32'};
...
exthdr=[];
for l=1:hdr.nsecs
    exthdr=[exthdr;fread(a,8,"int32",0,endian)];
    exthdr=[exthdr;fread(a,32,"float32",0,endian)];
end
...
buf=eval(strcat(dt,'(zeros(len,1));')); %good to pre-clear memory--can't believe I didn't do until now, 020120705pmc -- and now it's done properly, 020190725pmc
