...
if [ ! -f $zsname ]
then
/opt/bin/chromatic-shift "${datafilename}" > $tmpf
sh ${tmpf}
fi
...
Threshold $zsname ${zsname}.thr -z1=$Z -w1=0 -not_below=$THRS -result=mask;
2DObjFinder ${zsname}.thr -poly=$polyfilename -border=$BORDER -spacing=4 \
    -minpts=300 -exclude_edge -outer_only
perl /opt/bin/copyObjsFromPerims.pl "${zsname}" "${polyfilename}" $BORDER > $tmpf
sh ${tmpf}
